...
class TestBotGameServerCommunication:
    ...
    @pytest.mark.asyncio
    async def test_bot_server_integration_with_game_server(self, server_manager, test_room):
        """Test full integration between bot server and game server."""
        # Spawn bot through bot server
        bot_config = BotConfig(
            bot_type=BotType.RULES_BASED,
            difficulty=DifficultyLevel.INTERMEDIATE,
        ...
        try:
            # Wait for bot to initialize and connect
            ...
            # The bot should be in the room
            room_bots = server_manager.bot_server.get_room_bots(test_room["room_id"])
            assert len(room_bots) == 1
            assert room_bots[0]["bot_id"] == bot_id
            
            # Test bot configuration change
            success = await server_manager.bot_server.configure_bot_difficulty(
                bot_id, DifficultyLevel.EXPERT
            )
            assert success is True
            
            # Verify difficulty change
            updated_status = server_manager.bot_server.get_bot_status(bot_id)
            assert updated_status["config"]["difficulty"] == DifficultyLevel.EXPERT.value
    ...
    @pytest.mark.asyncio
    async def test_bot_reconnection_after_disconnect(self, server_manager, test_room):
        """Test bot reconnection capabilities after network disconnect."""
        # This test simulates bot reconnection logic
        bot_config = BotConfig(
            bot_type=BotType.RULES_BASED,
            difficulty=DifficultyLevel.INTERMEDIATE,
            name="ReconnectionTestBot"
        )
        ...
        try:
            ...
            bot_status = server_manager.bot_server.get_bot_status(bot_id)
            assert bot_status["status"] in [BotStatus.ACTIVE.value, BotStatus.CONNECTING.value]
            
            # Simulate connection loss by forcing client disconnect
            bot_instance = server_manager.bot_server._bots[bot_id]
            if bot_instance.game_client and bot_instance.game_client.websocket:
                await bot_instance.game_client.websocket.close()
            
            # Wait a moment for disconnect to be detected
            await asyncio.sleep(1)
            
            # Test bot health monitoring
            health_status = await server_manager.bot_server.monitor_bot_health(bot_id)
            assert health_status["bot_id"] == bot_id
            
            # Test reconnection attempt
            reconnect_success = await server_manager.bot_server.attempt_bot_reconnection(bot_id)
            
            # Note: Actual reconnection success depends on implementation details
            # The test validates that the reconnection attempt is handled gracefully
            assert isinstance(reconnect_success, bool)
            
        finally:
            await server_manager.bot_server.terminate_bot(bot_id)
    
    @pytest.mark.asyncio
    async def test_server_resource_limits(self, server_manager, test_room):
        """Test server behavior under resource limits."""
        bot_ids = []
        
        try:
            # Test spawning up to the limit
            max_bots = server_manager.bot_server.config.max_bots_per_room
            
            for i in range(max_bots):
                bot_config = BotConfig(
                    bot_type=BotType.RULES_BASED,
                    difficulty=DifficultyLevel.BEGINNER,
                    name=f"LimitTestBot_{i}"
                )
            ...
            with pytest.raises(RuntimeError, match="Maximum bots per room"):
                await server_manager.bot_server.spawn_bot(extra_bot_config, room_info)
            
            # Verify server status
            server_status = server_manager.bot_server.get_server_status()
            assert server_status["total_bots"] == max_bots
