...
class BotLifecycleEvent(Enum):
    ...
    BOT_EXITING_GAME_LOOP = "bot_exiting_game_loop"
...
class ConnectionHealth:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connection_duration": (
                str(self.connection_duration) if self.connection_duration else None
            ),
        }
...
class BotDiagnosticTracker:
    ...
    def __init__(
        self, max_events_per_bot: int = 1000, cleanup_interval_hours: int = 24
    ):
        """
        ...
        self._diagnostic_events: Dict[str, List[DiagnosticEvent]] = (
            {}
        )  # bot_id -> events
        ...
        self._health_callbacks: List[
            Callable[[str, BotDiagnosticInfo], Awaitable[None]]
        ] = []
    ...
    def _setup_diagnostic_logger(self) -> None:
        ...
        if not self.diagnostic_logger.handlers:
            ...
            self.diagnostic_logger.info(
                "Bot diagnostic logger initialized - file-only output"
            )
    ...
    def log_event(
        ...
        if len(self._diagnostic_events[bot_id]) > self.max_events_per_bot:
            self._diagnostic_events[bot_id] = self._diagnostic_events[bot_id][
                -self.max_events_per_bot :
            ]
    ...
    def update_bot_status(
        self, bot_id: str, status: str, error_message: Optional[str] = None
    ) -> None:
        """
        ...
        event_type = (
            BotLifecycleEvent.BOT_ERROR
            if status == "error"
            else BotLifecycleEvent.BOT_ACTIVE
        )
    ...
    def update_connection_status(
        ...
        event_type = (
            BotLifecycleEvent.BOT_WEBSOCKET_FAILED
            if connection_status == ConnectionStatus.FAILED
    ...
    def update_ai_status(
        self, bot_id: str, ai_status: AIStatus, error_message: Optional[str] = None
    ) -> None:
        """
        ...
        level = (
            DiagnosticLevel.ERROR
            if ai_status == AIStatus.ERROR
            else DiagnosticLevel.INFO
        )
        event_type = (
            BotLifecycleEvent.BOT_AI_LOAD_FAILED
            if ai_status == AIStatus.ERROR
    ...
    def record_bot_decision(
        self, bot_id: str, decision_details: Optional[Dict[str, Any]] = None
    ) -> None:
        """
    ...
    def record_bot_action(
        ...
        event_type = (
            BotLifecycleEvent.BOT_ACTION_FAILED
            if not success
            else BotLifecycleEvent.BOT_ACTION_SENT
        )
    ...
    def get_bot_diagnostics(self, bot_id: str) -> Optional[BotDiagnosticInfo]:
        ...
        diagnostic_info.uptime_seconds = (
            datetime.now() - diagnostic_info.created_at
        ).total_seconds()
    ...
    def get_activity_metrics(self, bot_id: str) -> Optional[BotActivityMetrics]:
        ...
        if metrics.decisions_made > 0:
            metrics.decision_success_rate = (
                metrics.decisions_made - metrics.errors_encountered
    ...
    def register_event_callback(
        self, callback: Callable[[DiagnosticEvent], Awaitable[None]]
    ) -> None:
        """Register a callback for diagnostic events."""
        self._event_callbacks.append(callback)

    def register_health_callback(
        self, callback: Callable[[str, BotDiagnosticInfo], Awaitable[None]]
    ) -> None:
    ...
    async def _notify_health_callbacks(
        self, bot_id: str, diagnostic_info: BotDiagnosticInfo
    ) -> None:
        ...
        while self._running:
            try:
                ...
                cleanup_threshold = current_time - timedelta(
                    hours=self.cleanup_interval_hours * 2
                )
