...
class MonitoredGameClient(GameClient):
    ...
    def _setup_diagnostic_logging(self) -> None:
        ...
        try:
            ...
            self.logger.exception("Failed to set up file logging")
    ...
    async def _periodic_flush_loop(self) -> None:
        ...
        try:
            ...
            if hasattr(self, "logger"):
                self.logger.exception("Error in periodic flush")
    ...
    async def connect(
        ...
        self.logger.debug(
            f"Connection details - WS URL: {self.ws_url}, HTTP URL: {self.http_url}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Successfully connected to room {room_code} in {connection_duration:.2f}s"
            )
            ...
            self.logger.exception(
                f"Failed to connect to room {room_code} after {connection_duration:.2f}s"
            )
            self.logger.debug(
                f"Error type: {type(e).__name__}, Failure reason: {failure_reason.value}"
            )
    ...
    async def send_keyboard_input(self, key: str, pressed: bool) -> None:
        ...
        try:
            self.logger.debug(
                f"Sending keyboard input: {key} {'pressed' if pressed else 'released'}"
            )
            await super().send_keyboard_input(key, pressed)

            # Track message sent
            message = json.dumps(
                {"type": "Key", "payload": {"key": key, "isDown": pressed}}
            )
            ...
            self.logger.exception(f"Failed to send keyboard input {key}")
    ...
    async def send_mouse_input(
        ...
        try:
            self.logger.debug(
                f"Sending mouse input: {button} {'pressed' if pressed else 'released'} at ({x}, {y})"
            )
            ...
            self.logger.exception(f"Failed to send mouse input {button} at ({x}, {y})")
            # Record failed action
            self.diagnostic_tracker.record_bot_action(
                bot_id=self.bot_id,
                action_type="mouse_input",
    ...
    async def exit_game(self) -> None:
        ...
        try:
            ...
            self.logger.exception("Error exiting game")
    ...
    async def enable_training_mode(
        ...
        self.logger.info(
            f"Enabling training mode - Speed: {speed_multiplier}x, Headless: {headless}"
        )
        if training_session_id:
            self.logger.debug(f"Training session ID: {training_session_id}")

        await super().enable_training_mode(
            speed_multiplier, headless, training_session_id
        )
    ...
    async def get_direct_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.diagnostic_tracker.record_bot_action(
                bot_id=self.bot_id, action_type="get_direct_state", success=True
            )

            return state

        except Exception as e:
            self.logger.exception("Failed to get direct state")
    ...
    def log_performance_metric(
        self, metric_name: str, value: Any, unit: Optional[str] = None
    ) -> None:
        """
    ...
    def log_connection_stats(self) -> None:
        ...
        try:
            ...
            if health:
                self.logger.info(
                    f"Connection health: {json.dumps(health, default=str)}"
                )
            ...
            self.logger.exception("Failed to log connection stats")
    ...
    def flush_logs(self) -> None:
        ...
        try:
            ...
            self.logger.exception(f"Failed to flush logs")
