...
from core.base_bot import BaseBot, Action
...
class BotInstance:
    ...
    bot_ai: Optional[BaseBot]  # RulesBasedBot or RL model
    created_at: datetime
...
class GameClientPool:
    ...
    async def get_client(
        self, bot_id: str, game_server_url: str
    ) -> MonitoredGameClient:
        """
        ...
        async with self._client_creation_lock:
            ...
            if self._available_clients:
                ...
                ws_url = (
                    game_server_url.replace("http://", "ws://").replace(
                        "https://", "wss://"
                    )
                    + "/ws"
                )
                client = MonitoredGameClient(
                    bot_id=bot_id, ws_url=ws_url, http_url=game_server_url
                )
        ...
        if bot_id in self._active_clients:
            ...
            try:
                ...
                if len(self._available_clients) < self.max_connections // 2:
                    ...
                    self.logger.debug(
                        f"Returned monitored client to pool from bot {bot_id}"
                    )
                else:
                    self.logger.debug(
                        f"Discarded monitored client from bot {bot_id} (pool full)"
                    )

            except Exception as e:
                self.logger.error(
                    f"Error returning monitored client from bot {bot_id}: {e}"
                )
    ...
    def get_pool_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "utilization_percent": int(
                (len(self._active_clients) / self.max_connections) * 100
            ),
        }
...
class BotServer:
    ...
    def __init__(self, config: BotServerConfig):
        ...
        # Event callbacks
        self._bot_status_callbacks: List[
            Callable[[str, BotStatus], Awaitable[None]]
        ] = []
    ...
    async def spawn_bot(self, bot_config: BotConfig, room_info: Dict[str, Any]) -> str:
        ...
        self.logger.info(
            f"Bot spawn requested - Type: {bot_config.bot_type.value}, "
        ...
        async with self._bot_creation_lock:
            try:
                ...
                self.logger.debug(
                    f"Current bot count: {current_bot_count}/{self.config.max_total_bots}"
                )
                ...
                if room_id:
                    ...
                    self.logger.debug(
                        f"Room {room_id} bot count: {room_bot_count}/{self.config.max_bots_per_room}"
                    )
                ...
                self.logger.debug(
                    f"Bot configuration validated successfully for {bot_id}"
                )
                ...
                self.logger.info(
                    f"Bot spawn setup completed successfully - ID: {bot_id}, "
                ...
                self.diagnostic_tracker.log_event(
                    ...
                    details={
                        ...
                        "room_bots_after": (
                            len(self._room_bots.get(room_id, set())) if room_id else 0
                        ),
                    },
                ...
                asyncio.create_task(
                    self._initialize_bot(bot_id, room_info, correlation_id)
                )
                ...
                spawn_duration = (datetime.now() - spawn_start_time).total_seconds()
                self.logger.error(
                    f"Bot spawn failed after {spawn_duration:.3f}s - "
    ...
    async def configure_bot_difficulty(
        self, bot_id: str, difficulty: DifficultyLevel
    ) -> bool:
        """
        ...
        try:
            ...
            if bot_instance.config.bot_type == BotType.RULES_BASED and isinstance(
                bot_instance.bot_ai, RulesBasedBot
            ):
                bot_instance.bot_ai.set_difficulty_level(difficulty)

                # Apply real-time difficulty adjustments
                await self._apply_realtime_difficulty_changes(
                    bot_id, old_difficulty, difficulty
                )
            ...
            self.logger.info(
                f"Updated bot {bot_id} difficulty from {old_difficulty.value} to {difficulty.value}"
            )
            return True
    ...
    def get_server_status(self) -> Dict[str, Any]:
        ...
        connection_stats = {
            ...
            "healthy_connections": len(
                [c for c in websocket_connections.values() if c.is_healthy]
            ),
            "failed_connections": len(
                [
                    c
                    for c in websocket_connections.values()
                    if c.connection_state.value == "failed"
                ]
            ),
            "connecting_connections": len(
                [
                    c
                    for c in websocket_connections.values()
    ...
    def register_bot_status_callback(
        self, callback: Callable[[str, BotStatus], Awaitable[None]]
    ) -> None:
        """Register a callback for bot status changes."""
        self._bot_status_callbacks.append(callback)

    def register_room_empty_callback(
    ...
    def get_bot_message_history(
        self, bot_id: str, limit: Optional[int] = None
    ) -> List[Any]:
        """
    ...
    async def check_room_human_players(self, room_id: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(
                f"Checking human players in room {room_id} (placeholder implementation)"
            )
            return True
    ...
    async def cleanup_room_bots(self, room_id: str, reason: str = "room_empty") -> int:
        ...
        self.logger.info(
            f"Cleaning up {len(bot_ids)} bots from room {room_id} (reason: {reason})"
        )
        ...
        for bot_id in bot_ids:
            try:
                ...
                self.logger.error(
                    f"Error cleaning up bot {bot_id} from room {room_id}: {e}"
                )
    ...
    async def monitor_bot_health(self, bot_id: str) -> Dict[str, Any]:
        ...
        time_since_activity = (
            current_time - bot_instance.last_activity
        ).total_seconds()
            ...
            health_status.update(
                {
                    "activity_metrics": {
                        ...
                        "last_decision_time": (
                            activity_metrics.last_decision_time.isoformat()
                            if activity_metrics.last_decision_time
                            else None
                        ),
                        "last_action_time": (
                            activity_metrics.last_action_time.isoformat()
                            if activity_metrics.last_action_time
                            else None
                        ),
                    }
                }
        ...
        if connection_health:
            health_status["connection_health"] = {
                ...
                "connection_duration": (
                    str(connection_health.connection_duration)
                    if connection_health.connection_duration
                    else None
                ),
            }
        ...
        health_status["healthy"] = (
            len(diagnostic_info.performance_issues) == 0
            and len(diagnostic_info.error_messages) == 0
        )
        ...
        if diagnostic_info.connection_status in [
        ...
        try:
            ...
            if bot_instance.config.training_mode:
                await bot_instance.game_client.enable_training_mode(
                    speed_multiplier=10.0, headless=True
                )
    ...
    async def _update_bot_status(
        self, bot_id: str, status: BotStatus, error_message: Optional[str] = None
    ) -> None:
        ...
        self.diagnostic_tracker.update_bot_status(
            bot_id=bot_id, status=status.value, error_message=error_message
        )
    ...
    async def _initialize_bot(
        ...
        try:
            ...
            self.logger.info(
                f"Starting bot initialization - ID: {bot_id}, "
            ...
            self.logger.debug(
                f"Loading AI for bot {bot_id} - Type: {bot_instance.config.bot_type.value}"
            )
            ...
            try:
                bot_instance.bot_ai = await self._load_bot_ai(
                    bot_instance.config, bot_id, correlation_id
                )
                ...
                self.logger.info(
                    f"AI loaded for bot {bot_id} in {ai_load_duration:.3f}s"
                )
                ...
                self.diagnostic_tracker.update_ai_status(
                    bot_id, AIStatus.ERROR, error_msg
                )
                self.diagnostic_tracker.log_event(
                    bot_id=bot_id,
                ...
                self.logger.error(
                    f"AI loading failed for bot {bot_id} after {ai_load_duration:.3f}s: {ai_error}"
                )
                raise ai_error
            ...
            websocket_url = (
                self.config.game_server_url.replace("http://", "ws://").replace(
                    "https://", "wss://"
                )
                + "/ws"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"Game client acquired for bot {bot_id} in {client_duration:.3f}s"
                )
                ...
                if bot_instance.config.training_mode:
                    ...
                    self.logger.debug(
                        f"Training mode enabled for bot {bot_id} in {training_duration:.3f}s"
                    )
                ...
                if room_code:
                    ...
                    self.diagnostic_tracker.log_event(
                        ...
                        details={
                            ...
                            "player_id": getattr(
                                bot_instance.game_client, "player_id", None
                            ),
                            "room_id": getattr(
                                bot_instance.game_client, "room_id", None
                            ),
                        },
                        correlation_id=correlation_id,
                    )
                    ...
                    self.logger.info(
                        f"Bot {bot_id} joined room {room_code} in {connect_duration:.3f}s"
                    )
                ...
                self.logger.error(
                    f"Connection failed for bot {bot_id} after {client_duration:.3f}s: {conn_error}"
                )
                raise conn_error
            ...
            self.logger.info(
                f"Bot {bot_id} initialization completed successfully in {total_init_duration:.3f}s"
            )
            ...
            self.logger.error(
                f"Bot {bot_id} initialization failed after {total_init_duration:.3f}s: {e}"
            )
    ...
    async def _load_bot_ai(
        self, config: BotConfig, bot_id: str, correlation_id: Optional[str] = None
    ) -> Any:
        ...
        try:
            if config.bot_type == BotType.RULES_BASED:
                self.logger.debug(
                    f"Loading rules-based AI for bot {bot_id} with difficulty {config.difficulty.value}"
                )
                ...
                try:
                    ...
                    if not hasattr(bot_ai, "difficulty"):
                        raise RuntimeError(
                            "Rules-based bot creation failed - missing difficulty attribute"
                        )

                    load_duration = (datetime.now() - load_start).total_seconds()
                    self.logger.debug(
                        f"Rules-based AI loaded successfully for bot {bot_id} in {load_duration:.3f}s"
                    )
                    ...
                    self.logger.error(
                        f"Rules-based AI loading failed for bot {bot_id} after {load_duration:.3f}s: {rules_error}"
                    )
                ...
                self.logger.debug(
                    f"Loading RL generation {config.generation} AI for bot {bot_id}"
                )
                ...
                if config.generation not in self._model_cache:
                    try:
                        self.logger.debug(
                            f"RL generation {config.generation} not in cache, attempting to load from model manager"
                        )
                        ...
                        if config.generation not in available_generations:
                            self.logger.warning(
                                f"RL generation {config.generation} not found in available models: {available_generations}"
                            )
                            self.logger.info(
                                f"Falling back to rules-based bot for bot {bot_id}"
                            )
                        ...
                        self.logger.warning(
                            f"RL model loading not fully implemented for generation {config.generation}"
                        )
                        self.logger.info(
                            f"Falling back to rules-based bot for bot {bot_id}"
                        )
                        ...
                        load_duration = (datetime.now() - load_start).total_seconds()
                        self.logger.debug(
                            f"Fallback rules-based AI loaded for bot {bot_id} in {load_duration:.3f}s"
                        )
                        ...
                        load_duration = (datetime.now() - load_start).total_seconds()
                        self.logger.error(
                            f"Failed to load RL model generation {config.generation} for bot {bot_id} after {load_duration:.3f}s: {e}"
                        )
                        self.logger.info(
                            f"Falling back to rules-based bot for bot {bot_id}"
                        )
                ...
                load_duration = (datetime.now() - load_start).total_seconds()
                self.logger.debug(
                    f"Cached RL model loaded for bot {bot_id} in {load_duration:.3f}s"
                )
            ...
            load_duration = (datetime.now() - load_start).total_seconds()
            self.logger.error(
                f"AI loading failed for bot {bot_id} after {load_duration:.3f}s: {e}"
            )
            raise

    def _validate_bot_config(
        self, config: BotConfig, room_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not isinstance(config.difficulty, DifficultyLevel):
                return {
                    "valid": False,
                    "error": f"Invalid difficulty level: {config.difficulty}",
                }
            ...
            if config.bot_type == BotType.RL_GENERATION:
                if config.generation is None:
                    return {
                        "valid": False,
                        "error": "Generation is required for RL bots",
                    }
            ...
            room_code = room_info.get("room_code")
            if room_code and (
                not isinstance(room_code, str) or len(room_code.strip()) == 0
            ):
                return {
                    "valid": False,
                    "error": "Room code must be a non-empty string if provided",
                }
    ...
    async def _run_bot_ai_loop(
        self, bot_id: str, correlation_id: Optional[str] = None
    ) -> None:
        ...
        try:
            while (
                bot_instance.status == BotStatus.ACTIVE
                and bot_instance.game_client
                and bot_instance.bot_ai
            ):

                # Get current game state
                if bot_instance.config.training_mode:
                    # TODO do we need direct state access?
                ...
                if bot_id in self.diagnostic_tracker._activity_metrics:
                    self.diagnostic_tracker._activity_metrics[
                        bot_id
                    ].game_state_updates += 1

                # Analyze game state and select action
                decision_start = datetime.now()
                action, analysis = bot_instance.bot_ai.process_state_and_get_action(
                    game_state
                )
                decision_time = (
            ...
            self.logger.exception(f"Error in bot AI loop for {bot_id}")
            await self._update_bot_status(bot_id, BotStatus.ERROR, str(e))
        ...
        self.diagnostic_tracker.log_event(
            bot_id=bot_id,
            event_type=BotLifecycleEvent.BOT_EXITING_GAME_LOOP,
            level=DiagnosticLevel.INFO,
            message="Bot exiting game loop",
            correlation_id=correlation_id,
        )

    async def _execute_bot_action(self, bot_id: str, action: Action) -> None:
    ...
    async def _apply_realtime_difficulty_changes(
        self,
        bot_id: str,
        ...
        try:
            ...
            if isinstance(bot_instance.bot_ai, RulesBasedBot):
                ...
                self.logger.debug(
                    f"Applied real-time difficulty changes to bot {bot_id}: "
                    f"frequency={new_frequency}, accuracy={new_accuracy}"
                )

        except Exception as e:
            self.logger.error(
                f"Error applying real-time difficulty changes to bot {bot_id}: {e}"
            )
        ...
        try:
            ...
            for room_id in rooms_to_check:
                ...
                if not has_human_players:
                    ...
                    if bots_to_cleanup:
                        self.logger.info(
                            f"Room {room_id} has no human players, cleaning up {len(bots_to_cleanup)} bots"
                        )
                        ...
                        for callback in self._room_empty_callbacks:
                            try:
                                ...
                                self.logger.error(
                                    f"Error in room empty callback for {room_id}: {e}"
                                )
        ...
        while self._running:
            try:
                ...
                timeout_threshold = current_time - timedelta(
                    seconds=self.config.bot_timeout_seconds
                )
                ...
                for bot_id, bot_instance in self._bots.items():
                    # Check for timeout
                    if (
                        bot_instance.last_activity < timeout_threshold
                        and bot_instance.status in [BotStatus.ACTIVE, BotStatus.ERROR]
                    ):
                    ...
                    if (
                        not health_status["healthy"]
                        and bot_instance.status == BotStatus.ACTIVE
                    ):
                ...
                for bot_id, health_status in unhealthy_bots:
                    if "connection_lost" in health_status.get("performance_issues", []):
                        self.logger.info(
                            f"Attempting reconnection for unhealthy bot {bot_id}"
                        )

                        # Try reconnection up to configured attempts
                        bot_instance = self._bots[bot_id]
                        reconnect_attempts = getattr(
                            bot_instance, "_reconnect_attempts", 0
                        )
                        ...
                        if reconnect_attempts < self.config.bot_reconnect_attempts:
                            ...
                            if success:
                                ...
                                self.logger.warning(
                                    f"Reconnection attempt {reconnect_attempts + 1} failed for bot {bot_id}"
                                )
                        else:
                            self.logger.error(
                                f"Bot {bot_id} exceeded max reconnection attempts, terminating"
                            )
                            inactive_bots.append(bot_id)
